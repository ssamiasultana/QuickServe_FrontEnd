...
import { useGetBookingReview } from '../../hooks/useReview';
import Card from '../ui/Card';
import Modal from '../ui/Modal';
import ReviewModal from '../Customer/ReviewModal';

// Review Button Component
function ReviewButton({ booking, onOpenReview }) {
  const { data: reviewData } = useGetBookingReview(booking.id, {
    enabled: booking.status === 'paid',
  });

  const hasReview = reviewData && reviewData.id;
                  ...
                  <div className='p-6'>
                    <div className='flex items-start justify-between gap-4'>
                      <div className='flex-1'>
                        ...
                        {/* Worker Information - Prominently Displayed for Admin and Customer */}
                        {(viewType === 'admin' || viewType === 'customer') && (
                    ...
                    {/* Action buttons section for customers */}
                    {viewType === 'customer' && (
                      <div className='flex items-center justify-end gap-2 pt-4 mt-4 border-t border-gray-200'>
                        {booking.status === 'paid' && booking.worker_id && (
                          <ReviewButton
                            booking={booking}
                            onOpenReview={() =>
